<template lang="html">
  <div class="">
    <!--Project-->
    <div class="project-cover-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-0.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-cover-h1">
        <h1>Publicitate în Facebook pentru sezonul doi la programului de slăbit „Preobrajenie”</h1>
    </div>
    <div class="project-objectiv">
        <div class="project-objectiv-name">
            <p>Obiectiv:&nbsp;să suplinim 15 locuri în programul de pierdere a kilogramelor în plus din fitness club Shantel.</p>
        </div>
    </div>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-1.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>În pregătirea, am primit o descriere a femeilor, căror li se potrivește programul de slăbit: femei cu vârste cuprinse între 22 și 45 de ani care trăiesc în Bălți și sunt interesate de fitness și de un stil de viață sănătos.</p>
    </div>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-2.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>Antrenorul principal ne-a trimis o descriere a programului de antrenament, metodele de slăbit, conținutul programelor de antrenament și programul alimentar pentru șase săptămâni — atât a durat programul de slăbit.</p>
    </div>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-3.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>Pentru a demonstra calitățile sportive a clubului, clienții și antrenorii au fost fotografiați în timpul antrenamentelor. La fel, am pregătit poze cu echipamentele, sălile și vestiarele.</p>
    </div>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-4.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>S-au scris șase texte despre program de slăbit. Despre beneficiile pentru clienți, datorită la ce vor pierde din greutate și cum își vor transforma corpul.</p>
    </div>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-5.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>S-a stabilit bugetul publicitar, data de expirare, s-au încărcat creativele și am pornit difuzarea anunțurilor. După trei zile de testare, anunțurile care au prezentat rezultate slabe au fost oprite.
        <br>Am lăsat texte despre detalii tehnice ale antrenamentului, despre obiectivul de transformare, despre tipurile de exerciții și despre jurnalul de nutriție. </p>
    </div>>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-6.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>În consecință, am suplinit două grupe și am oprit campania publicitară înainte de termen. Și acum povestea despre cum am creat și condus această campanie publicitară. Am încheiat campania publicitară înainte de termen — toate abonamentele au fost cumpărate provizoriu. </p>
    </div>
    <div class="project-img">
        <img src="@/static/shantel/preobrajenie/shantel-preobrajenie-7.png" class="lazyload" alt="альтернативный текст">
    </div>
    <div class="project-describe">
        <p>Aplicațiile au venit prin mesaje și comentarii pe Facebook, prin apeluri către administrator și vizite în club.</p>
    </div>
    <div class="carousel-cover">
      <div class="carousel">
        <agile :navButtons="false">
          <div class="carousel-img"><img src="@/static/shantel/preobrajenie/shantel-preobrajenie-grapgic-1.png" class="lazyload" alt="альтернативный текст"></div>
          <div class="carousel-img"><img src="@/static/shantel/preobrajenie/shantel-preobrajenie-grapgic-2.png" class="lazyload" alt="альтернативный текст"></div>
          <div class="carousel-img"><img src="@/static/shantel/preobrajenie/shantel-preobrajenie-grapgic-3.png" class="lazyload" alt="альтернативный текст"></div>
          <div class="carousel-img"><img src="@/static/shantel/preobrajenie/shantel-preobrajenie-grapgic-4.png" class="lazyload" alt="альтернативный текст"></div>
          <div class="carousel-img"><img src="@/static/shantel/preobrajenie/shantel-preobrajenie-grapgic-5.png" class="lazyload" alt="альтернативный текст"></div>
        </agile>
      </div>
    </div>
    <div class="project-describe">
        <p>Antrenorul principal ne-a trimis o descriere a programului de antrenament, metodele de slăbit, conținutul programelor de antrenament și programul alimentar pentru șase săptămâni — atât a durat programul de slăbit. </p>
    </div>

    <!-- Next project -->
    <div class="next-project">
        <div class="next-project-title">
            <p>Următorul proiect</p>
        </div>
        <div class="next-project-name">
            <a href="/projects/7_gurmania/index.html">Logo pentru „Gurmania”</a>
        </div>
        <div class="next-project-describe">
            <p>Noul logo pentru brandul de produse alimentare, sucuri ambalate și operator de comerț cu ridicata al conservelor Centaur.</p>
        </div>
    </div>
  </div>
</template>

<script type="text/javascript">
export default {}
</script>

<style lang="css">

.carousel-cover {
  width: 70%;
  margin: 0 15%;
}

.carousel-img {
  display: flex;
  align-self: center;
  margin: 100px 0 0 0;
  justify-content: center;
}

.carousel-img > img {
  max-width: 100%;
  height: auto;
}

.agile {
    margin-top: 20px;
    }

.agile__actions {
    margin-top: 20px;
    }

.agile__nav-button {
  background: transparent;
  border: none;
  color: #ccc;
  cursor: pointer;
  font-size: 24px;
  transition-duration: .3s;
  }

.agile__nav-button:hover {
  color: #888;
  }

.agile__dot {
  margin: 0 10px;
  }

.agile__dot button {
  background-color: #eee;
  border: none;
  border-radius: 50%;
  cursor: pointer;
  display: block;
  height: 20px;
  font-size: 0;
  line-height: 0;
  margin: 0;
  padding: 0;
  transition-duration: .3s;
  width: 20px;
  }

.agile__dot--current button {
  background-color: #0466c8;
  }

/** Design project **/

.project-cover {
    display: flex;
    flex-direction: column;
    margin: 100px 0 0 0;
}

.project-cover-h1 {
    margin: 0 5%;
}

.project-cover-h1 > h1 {
    font-family: Lora, sans-serif;
    font-size: 50px;
    font-weight: 400;
    line-height: 1.2;
    margin: 0;
    text-align: center;
    color: white;
    padding: 36px 0 36px 0;
    margin-block-start: 0em;
    margin-block-end: 0em;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
}

.project-objectiv {
    display: flex;
    flex-direction: row;
    margin: 0 10% 36px 10%;
    justify-content: center;
}

.project-objectiv-name > p {
    font-family: Lora, sans-serif;
    font-size: 30px;
    font-weight: 400;
    font-style: italic;
    line-height: 1.2;
    margin: 0;
    text-align: center;
    color: white;
    padding: 0 15% 36px 15%;
    margin-block-start: 0em;
    margin-block-end: 0em;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
}

.project-cover-img {
    display: flex;
    align-self: center;
    margin: 100px 0 0 0;
    justify-content: center;
}

.project-cover-img > img {
    max-width: 100%;
    height: auto;
}

.project-img {
    display: flex;
    align-self: center;
    margin: 72px 0 0 0;
    justify-content: center;
}

.project-img > img {
    max-width: 100%;
    height: auto;
}

.project-describe {
    margin: 36px 15% 72px 15%;
}

.project-describe > p {
    color: white;
    font-family: Lora, sans-serif;
    font-size: 30px;
    margin-block-start: 0;
    margin-block-end: 0;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
}

.last-img {
    margin: 0 0 72px 0;
}

@media (max-width: 768px) {
    .project-cover-h1 > h1 {
        font-size: 40px;
    }
    .project-objectiv-name > p {
        font-size: 25px;
    }
    .project-describe {
        margin: 36px 10% 72px 10%;
    }
    .project-describe > p {
        font-size: 25px;
    }
}

@media (max-width: 425px) {
    .project-cover-h1 > h1 {
        font-size: 30px;
        padding: 36px 0 18px 0;
    }
    .project-objectiv-name > p {
        font-size: 20px;
    }
    .project-describe > p {
        font-size: 20px;
    }
}

/** Project carousel **/

.project-carousel {
    width: 90%;
    margin: 72px 5% 0 5%;
}


/** Next project **/

.next-project {
    display: flex;
    flex-direction: column;
    padding: 72px 5%;
    border-top: 2px solid #636363;
    border-bottom: 2px solid #636363;
}

.next-project-title > p {
    font-family: Lora, sans-serif;
    font-size: 30px;
    font-weight: 400;
    line-height: 1.2;
    margin: 0;
    text-align: center;
    color: white;
    padding: 0 0 36px 0;
    margin-block-start: 0em;
    margin-block-end: 0em;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
}

.next-project-name {
    margin: 0 0 36px 0;
    text-align: center;
}

.next-project-name > a {
    font-size: 50px;
    font-family: Lora, sans-serif;
    color: white;
    font-weight: 400;
    cursor: pointer;
    border-bottom: 2px solid #636363;
    text-decoration: none;
    text-align: center;
    margin-block-start: 0em;
    margin-block-end: 0em;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
}

.next-project-describe > p {
    font-family: Lora, sans-serif;
    font-size: 30px;
    font-weight: 400;
    font-style: italic;
    line-height: 1.2;
    margin: 0;
    text-align: center;
    color: white;
    padding: 0 15%;
    margin-block-start: 0em;
    margin-block-end: 0em;
    margin-inline-start: 0px;
    margin-inline-end: 0px;
}

@media (max-width: 768px) {
    .next-project-title > p {
        font-size: 25px;
    }
    .next-project-name > a {
        font-size: 40px;
    }
    .next-project-describe > p {
        font-size: 25px;
    }
}

@media (max-width: 425px) {
    .next-project-title > p {
        font-size: 20px;
    }
    .next-project-name > a {
        font-size: 30px;
    }
    .next-project-describe > p {
        font-size: 20px;
        padding: 0;
    }

}
</style>
